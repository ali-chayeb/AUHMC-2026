/* ====== AUHMC 2026 — Admin Panel (Standalone for GitHub Pages) ====== */
...
(function () {
  ...
  const DEFAULTS = {
    ...
    stats: {
      ...
      lectures: '40+',
      participants: '300+',
      description: 'ينطلق المؤتمر العلمي الأول لمشفى حلب الجامعي (AUHMC 2026) ليجمع الأطباء الاختصاصيين، المقيمين، والكوادر التمريضية من مختلف المشافي التعليمية والصحية في سورية، في محفل أكاديمي رائد يهدف إلى تبادل الخبرات، عرض أحدث المستجدات الطبية، وتطوير المهارات السريرية عبر محاضرات وورش عمل تفاعلية.'
    },
    tracks: [
  ...
  // ====== REGISTRATIONS VIEWER (localStorage) ======
  function loadRegistrations() {
    ...
    try {
      ...
      container.innerHTML = `
            ...
            <tbody>
              ${stored.map((r, i) => `
                <tr style="border-bottom:1px solid var(--border);">
                  ...
                  <td style="padding:0.75rem;">${(r.workshops || []).length}</td>
        ...
        <p style="margin-top:1rem;color:var(--gray-500);font-size:0.85rem;">إجمالي المسجلين: ${stored.length}</p>
      `;
    } catch (e) {
      container.innerHTML = '<p style="color:var(--danger);text-align:center;padding:2rem;">خطأ في تحميل البيانات</p>';
    }
  }
