...
(function () {
  ...
  // ====== ADMIN DATA BRIDGE (localStorage fallback) ======
  ...
  // ====== DATA (will be populated from API or fallback) ======
  ...
  let WORKSHOPS = [];
  let SPONSORS = [];
  let THEME = { primary: '#002366', gold: '#D4AF37' };
  let HERO = {};
  let STATS = {};
  let FOOTER = {};
  let bgImage = null;
  ...
  async function fetchData() {
    try {
      ...
      console.warn('⚠️ API not available, using localStorage/defaults');
    }
    // Fallback to localStorage
    const adminData = loadAdminData();
    if (adminData) {
      applyLocalData(adminData);
      return true;
    }
  ...
  function applyRemoteData(data) {
    if (data.hero) {
      HERO = data.hero;
      if (data.hero.date) {
        CONFERENCE_DATE = new Date(data.hero.date + ':00+03:00');
      }
    }
    if (data.stats) STATS = data.stats;
    if (data.tracks) TRACKS = data.tracks;
    if (data.schedule) SCHEDULE = data.schedule;
    if (data.committees) COMMITTEES = data.committees;
    if (data.workshops) WORKSHOPS = data.workshops;
    if (data.sponsors) SPONSORS = data.sponsors;
    if (data.theme) THEME = data.theme;
    if (data.footer) FOOTER = data.footer;
  }

  function applyLocalData(adminData) {
    if (adminData.hero) {
      HERO = adminData.hero;
    ...
    if (adminData.stats) STATS = adminData.stats;
    if (adminData.tracks) TRACKS = adminData.tracks;
    if (adminData.schedule) SCHEDULE = adminData.schedule;
    if (adminData.committees) COMMITTEES = adminData.committees;
    if (adminData.workshops) WORKSHOPS = adminData.workshops;
    if (adminData.sponsors) SPONSORS = adminData.sponsors;
    if (adminData.theme) THEME = adminData.theme;
    if (adminData.footer) FOOTER = adminData.footer;
      ...
      regForm.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            alert(err.error || 'حدث خطأ في التسجيل. حاول مرة أخرى.');
          }
        } catch (e) {
          // Fallback: show success anyway if server is down
          regForm.style.display = 'none';
          const successEl = document.getElementById('regSuccess');
          if (successEl) successEl.style.display = 'block';
        }
